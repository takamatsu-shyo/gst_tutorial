...
static void tags_cb (GstElement *playbin, gint stream, CustomData *data) {
	gst_element_post_message (playbin,
		gst_message_new_application (GST_OBJECT (playbin),
			gst_structure_new_empty ("tags-changes")));
}
...
static void analyze_streams (CustomData *data) {
	...
	for (i = 0; i < n_audio; i++) {
		...
		g_signal_emit_by_name (data->playbin, "get-audio_tags", i, &tags);
		if (tags) {
...
static void application_cb (GstBus *bus, GstMessage *msg, CustomData *data) {
 	if (g_strcmp0 (gst_structure_get_name (gst_message_get_structure (msg)), "tag-changes") == 0) {
...
int main(int argc, char *argv[]) {
	...
	if (gtkglsink != NULL && videosink != NULL) {
		g_printerr ("Sccessfully crated GTK GL Sink");
	...
	if (!data.playbin || !videosink) {
		g_printerr ("Not all elements could be crated.\n");
		return -1;
	}
	...
	g_signal_connect (G_OBJECT (bus), "message::stage-changed", (GCallback)state_changed_cb, &data);
